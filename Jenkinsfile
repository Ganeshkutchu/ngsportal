pipeline {
    ...
    stages {
        ...
        stage('Upload Build Artifact') {
                    steps {
                        nexusArtifactUploader artifacts: [[artifactId: 'Ngs-Job-Portal', classifier: '', file: 'target/Ngs-Job-Portal-0.0.1-SNAPSHOT.jar', type: 'jar']], credentialsId: 'Nexus_Credentials', groupId: 'in.ngs', nexusUrl: '43.204.220.199:8081', nexusVersion: 'nexus3', protocol: 'http', repository: 'Ngs_snapshot_repo', version: '1.0-SNAPSHOT'
                    }
                }
            }
        }
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user ganesh:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-13-126-87-53.ap-south-1.compute.amazonaws.com:8080/job/NgsJobPortal-cd/buildWithParameters?token=github'"
                }
            }
       }
    }
